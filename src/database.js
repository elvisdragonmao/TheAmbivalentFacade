...
const invitationDb = {
  ...
  search(query) {
    const stmt = db.prepare(
      'SELECT * FROM invitations WHERE name LIKE ? OR slug LIKE ? ORDER BY created_at DESC'
    );
  ...
  create(data) {
    ...
    const stmt = db.prepare(
      'INSERT INTO invitations (slug, name, pronoun, message) VALUES (?, ?, ?, ?)'
    );
  ...
  update(id, data) {
    const stmt = db.prepare(
      'UPDATE invitations SET name = ?, pronoun = ?, message = ?, slug = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
    );
